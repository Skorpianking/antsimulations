 ...
 *  
 *  Edited by:  David King, 20 Sep 2016
 *  ========================================================
 */
...
public class Vector2D {
  ...
  public void limit (Vector2D vec, double limitedFactor) {
	   vec = vec.normalize();
	   vec.scale(limitedFactor);
  }
  
  static public double dist(Vector2D position, Vector2D target) {
	  return Math.pow(( Math.pow((position.dX - target.dX),2) + Math.pow((position.dY - target.dY),2)),.5);
  }
